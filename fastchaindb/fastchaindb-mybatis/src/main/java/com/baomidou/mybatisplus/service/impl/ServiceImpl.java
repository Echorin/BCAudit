...
public class ServiceImpl<M extends BaseMapper<T>, T> implements IService<T> {
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public boolean insertIntoChian(T entity) {

        boolean bFlag = retBool(baseMapper.insert(entity));

        //modify by lww at 2019.11.05,增加区块链接口
        if(bFlag){

            TransactionSendDTO transactionSendDTO=new TransactionSendDTO();
            transactionSendDTO.setData("123456");//GsonUtil.gsonString(entity)
            ...
            ResultDTO<String> resultDto = fastChainDBClient.sendTransaction(transactionSendDTO);
